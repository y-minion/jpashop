...
public class ItemController {
    ...
    public String create(BookForm form) {
        // 컨트롤러 메서드 파라미터에 객체를 적어두면, 요청 파라미터들을 이 객체에 자동으로 바인딩해준다.
        // form HTML에서 받은 정보에 대한 엔티티? 객체가 POST 매핑 메서드의 파라미터로 전달된다.

        //받아온 정보를 Book 엔티티에 넣어서 DB에 저장한다.
        Book book = new Book();
    ...
    //PathVariable 어노테이션이 있어야 url의 동적변수로 꺼내서 파라미터로 사용할 수 있다.
    public String updateItemForm(@PathVariable("itemId") Long itemId, Model model) {
        //먼저 수정하려는 아이템의 id를 조회한뒤, 받아온 데이터를 폼에 채워서 렌더링 한다.
